...
def _skip_background_daemon() -> bool:
    """Vercel / Lambda 等无长驻进程：后台 60s 线程无效；也可设 MARKET_SKIP_DAEMON=1 强制跳过。"""
    if os.environ.get("MARKET_SKIP_DAEMON", "").strip().lower() in ("1", "true", "yes"):
        return True
...
def _startup() -> None:
    if _skip_background_daemon():
        logger.info("Serverless 环境：跳过 unified_daemon_loop（请用容器/长进程跑完整引擎）")
        return
...
def root() -> Dict[str, Any]:
    """避免部署后访问根路径得到 404 `detail Not Found`（FastAPI 默认未注册路由）。"""
    return {
        "ok": True,
        "service": "market-skills",
        "docs": "/docs",
